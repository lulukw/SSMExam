...
public class LoginController {
	...
	public String do_login(@RequestParam(value = "name") String name,
		...
		if (c.length <= 0) {
			return "redirect:/component/message.jsp?message=" + URLEncoder.encode("登录失败", "UTF-8")
					+ "&redir=/SpringMVCExam/821/login.html";
		}
	...
	public String do_logout(HttpSession session) throws UnsupportedEncodingException {
		session.invalidate();
		return "redirect:/component/message.jsp?message=" + URLEncoder.encode("注销成功", "UTF-8")
				+ "&redir=/SpringMVCExam/821/login.html";
	}
}
