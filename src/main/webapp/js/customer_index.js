...
function getAndBindCustomers(page) {
	var url = "/SpringMVCExam/customer/ajaxGetCustomerInPage";
	var args = {
		"pageNum" : "" + page
	};
...
function getCustomerCount() {
	var posturl = "/SpringMVCExam/customer/ajaxGetCustomerCount";
	$.ajax({
		type : "post",
		url : posturl,
		async : false,
...
function do_del(cid) {
	if (confirm("删除后无法找回，是否继续？")) {
		var url = '/SpringMVCExam/customer/ajaxDeleteCustomer';
		$.ajax({
			type : "post",
			url : url,
			async : false,
...
function do_edit(cid) {
	var url = '/SpringMVCExam/customer/ajaxGetCustomer';
	var c;
	$.ajax({
		type : "post",
		url : url,
...
function edit_check(){
	...
	var url='/SpringMVCExam/customer/ajaxEditCustomer';
	$.ajax({
		type : "post",
		url : url,
		async : false,
